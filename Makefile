...
$(STAMPS)/sgs-stamp: 
	(cd illumos-gate && \
	 $(BLDENV) ../env/aarch64$(ENV_SUFFIX) 'cd usr/src/; make -j $(MAX_JOBS) bldtools sgs' && \
...
$(STAMPS)/crt-stamp: sgs gcc
	(cd illumos-gate && \
	$(BLDENV) ../env/aarch64$(ENV_SUFFIX) 'cd usr/src/lib/crt; make install' && \
...
$(STAMPS)/libc-stamp: ssp_ns gcc
	(cd illumos-gate && \
	$(BLDENV) ../env/aarch64$(ENV_SUFFIX) 'cd usr/src/lib/libc; make install' && \
...
$(STAMPS)/libm-stamp: ssp_ns gcc
	(cd illumos-gate && \
	$(BLDENV) ../env/aarch64$(ENV_SUFFIX) 'cd usr/src/lib/libm_aarch64; make install' && \
	mkdir -p $(SYSROOT)/usr/lib && \
...
$(STAMPS)/libsocket-stamp: libnsl ssp_ns gcc
	(cd illumos-gate && \
	$(BLDENV) ../env/aarch64$(ENV_SUFFIX) 'cd usr/src/lib/libsocket; make install' && \
	mkdir -p $(SYSROOT)/usr/lib && \
...
$(STAMPS)/libkstat-stamp: libc ssp_ns gcc
	(cd illumos-gate && \
	$(BLDENV) ../env/aarch64$(ENV_SUFFIX) 'cd usr/src/lib/libkstat; make install' && \
	mkdir -p $(SYSROOT)/usr/lib && \
...
$(STAMPS)/libnsl-stamp: libmp libmd libc ssp_ns gcc
	(cd illumos-gate && \
	$(BLDENV) ../env/aarch64$(ENV_SUFFIX) 'cd usr/src/lib/libnsl; make install' && \
	mkdir -p $(SYSROOT)/usr/lib && \
...
$(STAMPS)/libmd-stamp: libc ssp_ns gcc
	(cd illumos-gate && \
	$(BLDENV) ../env/aarch64$(ENV_SUFFIX) 'cd usr/src/lib/libmd; make install' && \
	mkdir -p $(SYSROOT)/usr/lib && \
...
$(STAMPS)/libmp-stamp: libc ssp_ns gcc
	(cd illumos-gate && \
	$(BLDENV) ../env/aarch64$(ENV_SUFFIX) 'cd usr/src/lib/libmp; make install' && \
	mkdir -p $(SYSROOT)/usr/lib && \
...
$(STAMPS)/ssp_ns-stamp: gcc
	(cd illumos-gate && \
	$(BLDENV) ../env/aarch64$(ENV_SUFFIX) 'cd usr/src/lib/ssp_ns && make install' && \
	mkdir -p $(SYSROOT)/usr/lib && \
...
$(STAMPS)/libc-filters-stamp: libc gcc
	(cd illumos-gate && \
	$(BLDENV) ../env/aarch64$(ENV_SUFFIX) 'cd usr/src/lib/librt && make install' && \
	$(BLDENV) ../env/aarch64$(ENV_SUFFIX) 'cd usr/src/cmd/sgs/libdl && make install' && \
	$(BLDENV) ../env/aarch64$(ENV_SUFFIX) 'cd usr/src/lib/libpthread && make install' && \
	mkdir -p $(SYSROOT)/usr/lib && \
...
$(STAMPS)/illumos-stamp: setup
	(cd illumos-gate && \
	 $(BLDENV) ../env/aarch64$(ENV_SUFFIX) 'cd usr/src; make -j $(MAX_JOBS) setup' && \
	 $(BLDENV) ../env/aarch64$(ENV_SUFFIX) 'cd usr/src; make -j $(MAX_JOBS) install') && \
	touch $@

illumos-pkgs: $(STAMPS)/illumos-pkgs
$(STAMPS)/illumos-pkgs:
	(cd illumos-gate && \
	 $(BLDENV) ../env/aarch64$(ENV_SUFFIX) 'cd usr/src/pkg; make -j $(MAX_JOBS) install') && \
	touch $@
